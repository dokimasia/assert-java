...
///
/// Every assertion takes a seat first and a message last. The message states the
/// contract under test and is the first line of the failure, so a failure says what was
...
/// ```
///
/// [Soft] carries the same assertions under the same names and runs the same
/// comparison. Only what happens on failure differs.
///
...
public final class Check {
  ...
  private static final Mode MODE = Mode.FATAL;
  ...
  /// single property the static form reads better. The chain stops the test at the first failing method.
  ///
  ...
  /// the test. A body that passes is the failure.
  ///
  /// On the aborting surface only: the recording surface cannot drive a check to
  /// failure, because it does not stop.
  ///
  /// @param seat where the failure is reported
  /// @param msg the contract under test
  /// @param body expected to report a failure against the recorder it is handed
  /// @return the failure the body produced, so its text can be asserted on
  public static String rejects(Seat seat, String msg, Consumer<Recorder> body) {
    seat.helper();
    Recorder recorder = new Recorder();
    body.accept(recorder);

    if (!recorder.failed()) {
      seat.fail(msg + ": the body reported no failure");
      return "";
    }
